...
class Settings(BaseSettings):
    ...
    # ── TTS 계정 한도 (스펙 13 §C-0) ────────────────────────────────
    # ElevenLabs 구독 플랜의 월 포함 크레딧. **1 크레딧 = 1자**다.
    # 대시보드의 "포함된 분"은 영어(약 1,000자/분) 기준이라 한국어(300~350자/분)에는
    # 약 3배 유리하다 — 우리 계산은 전부 문자 수 기준이므로 이 값을 그대로 쓴다.
    #
    # ⚠️ **이 값은 하드캡이 아니다.** usage-based billing 이 ON 이라 초과분($0.18/분)이
    # 그대로 청구된다. 계정 차원의 상한이 없으므로 교수자별 쿼터와 전역 브레이커가
    # 유일한 방어선이다. 이 값은 "얼마나 초과하고 있는가"를 보여주기 위한 기준선일 뿐.
    #
    # 2026-07-26 확인: Creator $22/월 = 323,670 크레딧(갱신 8/13).
    ELEVENLABS_MONTHLY_CREDITS: int = 323_670

    # ── TTS 상한 (스펙 13 §C-3) ──────────────────────────────────────
    # **1차 방어선 — 교수자별 월 문자 쿼터.**
    # 전역 $ 브레이커를 1차로 쓰지 않는 이유: 우리는 콘솔에 "한 명 때문에 전원의 아바타
    # 제작이 동시에 멈춥니다"라고 써 놨다. TTS 에 전역 브레이커만 달면 교수 한 명이
    # 실험하다 한도를 태울 때 나머지가 학기 중에 강의를 못 만든다.
    #
    # 30,000자 = 강의 약 3편(슬라이드 20장 × 500자). 8월 베타는 교수가 도구를 시험하는
    # 단계지 학기 강의를 다 만드는 게 아니다. 부족하면 운영자가 개별 상향한다
    # (POST /api/v1/admin/users/{id}/reset-tts-quota).
    #
    # ⚠️ 20명 × 3만 = 60만 자로 ElevenLabs Creator 한도(32만)의 1.9배다. 이건 의도된
    # 값이다 — 쿼터의 목적은 지출을 0 으로 만드는 게 아니라 **최악을 계산 가능하게**
    # 만드는 것이다. 지금은 상한이 없어 최악을 계산할 수조차 없다.
    TTS_MONTHLY_CHARS_PER_INSTRUCTOR: int = 30_000

    # **2차 방어선 — 전역 $ 브레이커.** 재시도 루프·버그성 대량 생성 같은 사고만 끊는다.
    # 정상 사용에서는 위 쿼터가 먼저 걸리므로 이 한도에 닿지 않아야 정상이다.
    # 0 이면 해당 검사 비활성.
    TTS_DAILY_BUDGET_USD: float = 200.0
    TTS_MONTHLY_BUDGET_USD: float = 1000.0
