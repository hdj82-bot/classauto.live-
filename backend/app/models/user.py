...
class User(Base):
    ...
    #   (다른 음성으로 다시 만들기 판정용).
    # photo_avatar_preview_text: 그 미리보기를 렌더할 때 읽힌 대본(스크립트).
    #   "룩과 목소리 아바타 제작" 빌더의 스크립트 테스트가 임의 문장을 렌더하므로,
    #   같은 (음성·대본) 조합은 캐시 적중시키고 대본이 바뀌면 다시 렌더하도록 키로 쓴다.
    #   NULL = 기본 샘플 문장(_PREVIEW_TEXT)으로 렌더된 캐시(과거 데이터 호환).
