...
### 핵심 차별점 4가지
1. **강의 범위 제한 Q&A** — 강의 자료(PPT+스크립트)를 근거로 **강의 관련 질문만** 답변, 무관한 일반 질문은 자동 거부 (2026-06-11부터 임베딩 임계값이 아니라 **Claude 관련성 판단**, `docs/planning/02-guardrails.md` §4)
...
- ❌ 학습자 화면을 라이트 모드로 만들기
- ❌ 학생 측에 무제한 Q&A 허용 (3차 빈도 한도 가드레일 위반)
- ❌ 강의와 무관한 일반 질문(일반 상식·잡담·범용 AI 대행)에 답변 허용 — 단, 강의가 다루는 주제와 관련된 질문은 강의 자료 밖이라도 전문가 수준으로 답함(2026-06-11 정책, Claude 관련성 판단)
...
- 2026-06-11: **학생 Q&A 범위 판정 전환** — 2차 가드레일을 임베딩 유사도 하드컷(0.7→0.4)에서 **Claude 관련성 판단**으로 변경(교수자 결정). 강의 자료(PPT+스크립트) 동시 참조, 강의 관련 질문은 자료 밖이라도 전문가 수준 답변, 무관 질문만 `[[OUT_OF_SCOPE]]` 거부. 목적(학습 외 차단) 유지·판정 정교화. `docs/planning/02-guardrails.md` §4 갱신.
