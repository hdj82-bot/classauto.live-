...
#### D-1. 진입점이 없으면 맥락도 없다 — 후속 조치 완료 *(2026-07-27)*

§D 를 하며 드러난 것: `GlobalFeedbackButton` 이 2026-06-27 부터 `app/layout.tsx` 에서
**내려가 있었다.** 즉 `POST /api/v1/feedback` 을 부르는 클라이언트가 하나도 없었고,
운영자 인박스는 8월 베타를 **텅 빈 채로** 시작할 참이었다.

프로덕션 확인(2026-07-27): `/api/v1/board/posts` → `total: 0`. 대체 창구로 지정됐던
공개 게시판도 한 달간 0건이었다. 즉 "살아 있는 다른 경로가 있다"는 전제가 성립하지
않았다.

**조치**: 고정 버튼을 복원하는 대신 진입점을 각 화면의 레이아웃 흐름 안으로 옮겼다.
배치 표·근거·`/board` 를 인박스에 합치지 않는 이유는 **스펙 13 §F-1 · §F-2** 에 있다.

- 교수자 일반 화면 → 사이드바 하단 슬롯
- 교수자 스튜디오 → ActionBar 좌측 슬롯 (이 화면은 사이드바가 없다)
- 학생 → `/dashboard` · `/c/[slug]` (**시청 화면 제외** — 집중도 방해)

`LectureContext` 는 `professor/layout.tsx` 에도 얹었고, 스튜디오 마법사가 라우트
param 으로 `lectureId` 를 즉시 등록한다. 그래서 **스튜디오에서 낸 제보에 `lecture_id`
와 `page` 가 함께 실린다** — §D 가 실제로 값을 채우는지는 이 조합에서만 확인되므로
회귀 테스트를 그 조합으로 걸었다.
