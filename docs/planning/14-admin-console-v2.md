...
3. **운영자 임퍼서네이션은 읽기 전용이다.** F 작업의 view-as 세션은 어떤 경로로도 쓰기를 허용하지
   않는다. "관리자니까 대신 고쳐준다"는 유혹을 코드에 남기지 말 것 — 콘솔의 쓰기 권한은 §5의
   화이트리스트 4개뿐이다.
...
```
- `/admin/users` 는 **`/admin/beta` 에 흡수**한다(역할 변경·유저 삭제는 베타 현황 행의 오버플로
  메뉴로). 나란히 두면 "테스터 목록"이 두 개가 된다.
- `/admin/testers/[id]` 는 드릴다운이므로 사이드바에 넣지 않는다.
- 미처리 이슈·피드백 수는 **사이드바 배지**로 노출(개요의 카드와 같은 소스).
...
콘솔에서 허용하는 상태 변경은 **아래 4개뿐**이다. 그 외 데이터 수정은 콘솔이 아니라 코드/DB로 한다.
...
**네 개 전부 감사 로그 1행을 남긴다.** 감사 로그가 못 따라가는 쓰기 기능은 추가하지 않는다.
