...
(기본 `False`) 뒤에 두고 아래 순서로만 켠다.

아래 **0 → 7** 을 위에서부터 그대로 밟는다. 하나라도 어긋나면 멈추고 켜지 않는다 —
켜는 순간 미등록 학생이 전부 403 이 된다.
...
| 무엇 | 확인 |
|---|---|
| Railway(백엔드) | `flag=False` 상태로 최신 커밋 배포 완료 |
| Vercel(프론트) | `join` 호출 + **딥링크 복귀 수정(#615)** 포함 커밋 배포 완료 |

> **#615 가 반드시 들어가 있어야 한다.** 그 전 빌드는 *이미 계정이 있는* 학생이 QR 을
> 찍어도 로그인 후 대시보드로 떨어져 `join` 이 호출되지 않았다(§7.5). 그 상태로 게이트를
> 켜면 **재학생 전원이 재생 불가인데 명단에도 안 잡혀** 원인을 찾을 수 없다.

---

### 1. 교수자 계정으로 Course QR 을 만든다

`/professor/learners` → 대상 강좌 카드 → **강좌 QR**. 링크가 `/c/{slug}` 형태인지 본다.

---

### 2. ⚠️ 시크릿 창에서 **이미 가입된** 학생 계정으로 QR 주소를 연다

**이 단계가 핵심이다.** 신규 가입 경로는 원래부터 딥링크가 복귀했으므로 검증 가치가 없다.
확인할 것은 **기존 계정 경로**다.

1. 시크릿 창(또는 다른 브라우저)에서 QR 주소를 연다
2. 로그인한다
3. **`/c/{slug}` 로 돌아오는가?** ← 여기서 `/dashboard` 로 떨어지면 **중단.** 0 을 다시 본다
4. 개발자도구 Network 에 `POST /api/v1/enrollments/join` 이 **200** 으로 있는가?

---

### 3. 그 학생이 명단에 잡히는지 확인

교수자 계정으로 `/professor/learners` → 그 강좌 → **수강 명단**. 방금 들어온 학생이
있어야 한다. 아직 안 봤으면 `최근 시청` 은 "시청 기록 없음"이 맞다 — 그게 정상이다.

여기까지 통과하면 **경로 전체가 실제로 도는 것이 확인된 것**이다. 이제 데이터를 본다.

---

### 4. 등록이 실제로 쌓이고 있는가 *(구 체크리스트 A)*

가장 중요하다. 안 쌓이는데 켜면 전원이 막힌다.
...
- `total = 0` → **켜지 마라.** 프론트 join 이 안 나가고 있다(0 을 다시 확인).
- `last_24h = 0` 인데 그 사이 학생 진입이 있었다면 → 마찬가지로 켜지 마라.

---

### 5. 세션이 있는데 등록이 없는 학생이 남아 있지 않은가 *(구 B)*

게이트를 켜는 순간 이들이 막힌다. 프론트 join 배포 이전부터 보던 기존 학생이 여기 걸린다.
...
### 6. 제적자가 의도한 사람뿐인가 *(구 C)*

켜면 이들만 정확히 막힌다. 의도하지 않은 이름이 있으면 명단 화면에서 **재등록**으로 되돌린다.
...
### 7. Railway 에서 `ENROLLMENT_GATE_ENABLED=true`

**켠 직후 볼 것**: 학생 문의·오류 로그에서 403 이 급증하는지. 급증하면 5 를 다시 확인한다.
...
> 1·2단계 배포만으로도 §1.2 의 관측 문제(누가 등록했는지 기록)는 해결된다. 게이트는
