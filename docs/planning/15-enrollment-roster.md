...
`enrollments` 테이블 + 인덱스 3개 + `courses.term` 컬럼을 **한 리비전**(`0072`)에 넣는다.

> 스펙 14 §C 가 `0072` 를 예약해 뒀었지만, C 는 아직 구현 전이고 이쪽이 먼저 머지된다.
> **미구현 작업에 번호를 예약하면 먼저 머지되는 쪽과 계속 어긋난다** — 착수 시점의 실제
> head 를 확인하고 그 다음 번호를 쓰는 것이 규칙이다. C 는 `0073` 으로 옮겨 적었다.
...
공유 화면 등으로 새어도 그것만으로는 아무것도 못 한다.
