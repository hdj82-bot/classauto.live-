...
강의 1편 = 슬라이드 20장 × 발화 300자 = **6,000자** 기준.

| 시나리오 | 월 강의 편수 | 월 문자 수 | 월 비용 |
|---|---|---|---|
| 과목 1개 (주1회) | 20명 × 4편 | 48만 자 | **$144** |
| 과목 2개 | 20명 × 8편 | 96만 자 | **$288** |
| 과목 3개 | 20명 × 12편 | 144만 자 | **$432** |
| 과목 3개 · 주2회 | 20명 × 24편 | 288만 자 | **$864** |

**교수는 한 학기에 과목 2~3개를 가르친다.** 재생성을 포함하면 현실적으로 **월 $600~1,700**,
9월 40명이면 **그 두 배**다. HeyGen 월 예산($600)과 같은 급이거나 더 크다.

⚠️ **발화 300자는 가정이고 실제는 더 길 가능성이 높다**(§C-4). 500자면 위 표가 전부 1.7배다.

⚠️ **재생성 시 전액 재과금된다**(§C-5). 위 표에 재생성 배수를 곱해야 한다.

##### ElevenLabs 구독 플랜이 1차 하드캡이다

`cost_tracker.py:28` 의 `$0.30/1k chars` 주석은 **"multilingual_v2 Starter 기준"** —
**최하위 플랜, 가장 비싼 단가**다. 상위 플랜은 1k 자당 실효 단가가 절반 이하로 떨어져,
**볼륨이 커지면 상위 플랜이 오히려 총액을 낮춘다.**

> **usage-based billing(초과 종량과금)이 OFF 인 것이 전제다.** OFF 라면 구독 플랜의 월
> 문자 할당량이 **진짜 하드캡**이고, 우리 브레이커는 그 안에서 배분을 관리하는 2차
> 장치다. ON 이면 할당량 초과분이 그대로 청구되므로 브레이커가 유일한 방어선이 된다.
> **플랜과 usage-based billing 설정을 먼저 확인할 것.**

| 플랜 | 월 요금 | 월 포함 문자 | 실효 단가/1k | 확인일 |
|---|---|---|---|---|
| Starter | (확인 필요) | (확인 필요) | **0.30** (코드 상수) | 2026-06? |
| Creator | | | | |
| Pro | | | | |
| Scale | | | | |

*(플랜 확인 후 채울 것. 실효 단가가 정해지면 `cost_tracker.ELEVENLABS_USD_PER_1K_CHARS`
를 그 값으로 맞추고 §C-3 쿼터 기본값을 재산정한다.)*
...
```
TTS_MONTHLY_CHARS_PER_INSTRUCTOR: int = 100_000   # 강의 약 16편 (6,000자/편 기준)
```
...
> ⚠️ 기본값 100,000 은 **발화 300자 가정**에 기댄 값이다. §C-4 실측이 500자로 나오면
> 강의 약 10편밖에 안 되므로 재산정해야 한다. §C-5(재생성 재과금)를 고치기 전이라면
> 재생성이 쿼터를 빠르게 태운다는 점도 감안할 것.
...
#### C-5. ⚠️ 재생성이 전액 재과금된다  *(2026-07-26 조사 — 심각)*

**"아무것도 안 바꾸고 다시 제작"해도 전 슬라이드 TTS 가 다시 과금된다.**
...
**조치 후보**(§C-3 과 함께 판단):
1. **텍스트 해시 재사용** — `VideoRender` 에 `script_text` 해시 컬럼을 두고, 같은
   `(lecture_id, slide_number, hash, voice_id, voice_speed)` 의 성공한 오디오가 있으면
   S3 키를 재사용한다. **근본 해결이고 절감 효과가 가장 크다**(아바타만 바꾸는 재제작은
   TTS 비용 0). 마이그레이션 1개 + 조회 1개.
2. 승인 시 직전 렌더와 세그먼트를 비교해 **변경된 슬라이드만** 새로 만든다. 1번의
   부분집합이라 1번을 하면 불필요.
3. 아무것도 안 하고 쿼터(§C-3)로만 막는다 — 재생성이 쿼터를 빠르게 태우므로 교수자가
   체감하는 제약이 커진다.

**1번을 권장한다.** 재렌더 상한(§C-2)이 "횟수"를 죄는 것과 달리, 이건 같은 작업의
**중복 과금 자체**를 없앤다.
