...
`0.0083` 이냐 `0.0167` 이냐는 **청구서로 확정한다.** 그때까지는 §C-1a 대로 보수적인
0.0167 로 통일한다.
...
월별로 `SUM(duration_seconds)` · `SUM(cost_usd)` · 추정단가를 HeyGen/VisionStory 로 나눠
출력한다. **읽는 법이 중요하다.**

- **"추정단가"는 순환 참조다.** `render_cost_logs.cost_usd` 자체가 `duration × 설정단가`
  로 저장된 값이라, `SUM(cost)/SUM(duration)` 은 설정단가를 되돌려줄 뿐이다. 쓸모는
  ① 그 달에 단가가 일관 적용됐는지 확인(중간에 바뀌었으면 혼합값), ② 진짜 단가의 **분모**
  를 얻는 것.
- **진짜 단가 ≈ (그 달 HeyGen 청구액 − Q&A 렌더분) ÷ SUM(duration_seconds)**
...
> `memo` 에 기록) 다음 확정 때는 청구서 대조가 훨씬 정확해진다. 지금 스키마로는 여기까지가
> 한계다.
