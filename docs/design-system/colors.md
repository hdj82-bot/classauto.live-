...
| 영역 | 베이스 | 포인트 | 비고 |
|---|---|---|---|
| 메인 사이트 (랜딩·features·pricing·use-cases·trust·security 등) | `#0A0A0A` 다크 | 골드 `#FFB627` | 무채색 + 그라데이션 메쉬. `MarketingShell` 이 5+ 페이지 호스트 |
...
메인 사이트 페이지군은 모두 다크 베이스를 공유한다 (`MarketingShell`). 골드는
페이지당 채움 1회 원칙 (§3) 그대로 — pricing 의 Basic 카드 CTA 같은 단일
강조 지점에만.

| 페이지 | 베이스 | 골드 사용 | 그라데이션 메쉬 | 의미적 컬러 |
|---|:---:|:---:|:---:|:---:|
| 랜딩 (`/`) | 다크 | ✓ 다수 | ✓ 배경 | ❌ |
| Features (`/features`) | 다크 | ✓ CTA | ✓ 부분 | ❌ |
| Pricing (`/pricing`) | 다크 | ✓ Basic 카드 | ❌ (단순함 우선) | ❌ |
| Demo (`/demo`) | 다크 | ✓ CTA·배지 | ✓ 미니 히어로 + CTA 모달 | ❌ |
| Use-cases / Trust / Security 등 | 다크 | ✓ CTA | 부분 | ❌ |
| 교수자 대시보드 | 라이트 | ✓ 활성 메뉴·CTA | ❌ | ✓ 차트 |
| 영상 제작 마법사 | 라이트 | ✓ 비용 미터 | ❌ | ✓ 한도 경고 |
| 학생 영상 시청 | 다크 | ✓ 익명 반응 | ❌ | ❌ |
| 집중 경고 | 다크 | ✓ 마스코트 글로우 | ❌ | ❌ |
...
| 2026-05-07 | §1·§8 표 갱신 — 메인 사이트 베이스 "라이트" → "다크" 정정. CLAUDE.md 핵심 디자인 원칙·`MarketingShell` 5+ 페이지 구현·R4 4창 합의와 일치시킴 (R4 PR 통합 시점). |
