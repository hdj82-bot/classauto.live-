  ...
  const handleGenerate = useCallback(async () => {
    ...
    if (approved) {
      ...
      if (genDone && videoId) {
        ...
        // 변경 없음 — 이미 완성된 답변을 다시 렌더하지 않는다(비용 0). 이전엔 여기서
        // 'Q&A 답변 강제 재생성'(ready 클립까지 전량 재렌더)을 제안했으나, 완성된
        // 답변까지 다시 만들어 VisionStory 크레딧을 낭비해 제거했다(2026-06-16 사용자
        // 결정). 특정 답변을 다시 만들려면 그 질문 카드를 삭제 후 다시 추가하면 그
        // 항목만 새로 렌더된다. (아바타·음성 변경은 PATCH 가 자동으로 seed 를 무효화.)
        if (nothingToDo) {
          // rendering 에 갇힌 강의 done 회복만 위해 rerender 한 번 호출(슬라이드 재사용=비용 0).
          try {
          ...
          toast(
            "점검 완료 — 변경된 부분이 없어요. 특정 답변을 다시 만들려면 그 질문 카드를 삭제 후 다시 추가하세요.",
            "success",
          );
          return;
        }
