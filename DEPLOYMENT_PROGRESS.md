...
### 미할당 갭 — React #418: 정적 분석으로 원인 확정 (2026-05-16 갱신)

이전 가설(StatCounter `toLocaleString()` SSR/hydration)은 **오진으로 종결**. 정적 분석 결과 원인 확정:

- **StatCounter 는 무관 (고아 컴포넌트)**: #117(전체 섹션 제거) 이후 어떤 페이지에서도 import·렌더되지 않음. 창 2 #147 이 StatCounter 를 locale 무관 결정적 함수로 고쳤으나 렌더 경로에 없어 #418 과 무관(예방적 정리로만 의미).
- **확정된 실제 원인 — `frontend/src/contexts/I18nContext.tsx` 의 `useSyncExternalStore` 로케일 스냅샷 불일치**: 서버 스냅샷(`getServerLocaleSnapshot`)은 항상 `'ko'` 를 반환하는데, 클라이언트 스냅샷(`getLocaleSnapshot`)은 `window.localStorage` 의 `ifl-locale` 값을 읽는다. 사용자 localStorage 로케일이 `'en'` 이면 **SSR=ko / 첫 CSR=en** 으로 첫 페인트 트리가 달라져 hydration mismatch(React #418) 발생. 단일 컴포넌트가 아니라 `I18nProvider` 가 감싸는 **전 페이지 공통 결함**이라 v2 재작성과 무관하게 잔존.
- **수정 진행**: 별도 레인 `fix/i18n-hydration-418` 에서 **진행 중**. 표준 패턴 = 서버·클라이언트 첫 스냅샷을 항상 `'ko'` 로 일치시키고, mount 후 `useEffect` 에서 저장된 로케일로 전환(첫 페인트 일치 보장). StatCounter 영역 신규 레인 배정은 **불필요로 종결**(원인 아님).

### 열린 PR (2026-05-16 현재 — 머지 대기/판단 필요)

| PR | 내용 | 상태 · 필요 조치 |
|---|---|---|
| [#66](https://github.com/hdj82-bot/classauto.live-/pull/66) | python 3.14 bump | ⏳ **도커 빌드 실검증 필요**. main 머지 전 `workflow_dispatch` 등으로 C-extension 휠(psycopg2-binary/Pillow/asyncpg/pgvector) 빌드 성공 확인 후 진행. 창 1 후속. |
| [#108](https://github.com/hdj82-bot/classauto.live-/pull/108) | Dependabot anthropic 0.30 → 0.100 | 코드 변경 불필요는 확인됨(#148). 스모크 테스트 변인 분리를 위해 **베이스라인 스모크 1회 후** 머지·재배포·재테스트 권장(상단 §"스모크 테스트 사전 점검"). |
| [#135](https://github.com/hdj82-bot/classauto.live-/pull/135) | (열림) | 분류·판단 필요 — 다음 세션에서 내용 확인 후 레인/처리 결정. |

### 운영 정리 메모 (2026-05-16)

- **브랜치 정리**: 머지 완료된 v2·후속·deps 작업 브랜치 등 **삭제 대기 52개**. GitHub repo **Settings → "Automatically delete head branches" 활성화 권장**(이후 머지 시 head 브랜치 자동 삭제). 현재 잔존 52개는 일괄 수동 삭제 1회 필요.
- **토큰**: 작업 편의로 채팅에 평문 노출됐던 GitHub PAT **revoke 완료**. 향후 자동화는 단명 토큰/`workflow_dispatch` 권장.
